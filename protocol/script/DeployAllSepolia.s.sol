...
contract DeployAllSepolia is Script {
    ...
    address constant TAR    = 0x9a9F2ccbFE2936223f312400DB614737f817c855;
    address constant TOKEN  = 0x9DbEB1bC4eB090a653F19d166eDE17A5BFFc896d;
    ChainId constant LOCAL  = ChainId.wrap(1);
    ChainId constant REMOTE = ChainId.wrap(2);
    ...
    function run() external {
        ...
        console.log("Coord: %s", vm.toString(address(coord)));
    ...
    function _salt(string memory label) internal view returns (bytes32) {
        return keccak256(abi.encode(label, uint256(1)));
    }
    ...
    function _identity(address ir, address ci, address user, uint16 country) internal {
        bytes32 s = keccak256(abi.encode("inv", user, uint256(1)));
        vm.startBroadcast(deployer);
    ...
    function _configure(ProtocolAccessManager pam, SettlementCoordinator coord, CCIPTransportProvider transport, ERC3643AssetAdapter adapter, RuleEngineComplianceProvider comp, address token) internal {
        ...
        comp.setChainJurisdiction(LOCAL, Config.JURISDICTION_US);
        comp.setChainJurisdiction(REMOTE, Config.JURISDICTION_EU);
        ...
        vm.startBroadcast(deployer);
        transport.setChainSelector(LOCAL, Config.SEPOLIA_CHAIN_SELECTOR);
        transport.setChainSelector(REMOTE, Config.FUJI_CHAIN_SELECTOR);
    ...
    function _fund(address transport) internal {
        ...
        (ok,) = payable(transport).call{value: 0.02 ether}(""); require(ok, "ETH xfer");
        vm.stopBroadcast();
    }
}
